...
void filesystem::get_filename(const char* pathname, char* filename, size_t size)
{
	if (file_exists(pathname))
	{
		fs::path path(pathname);
		strcpy_s(filename, size, path.filename().string().c_str());
	}
}

void filesystem::get_extension(const char* pathname, char* extension, size_t size)
{
	if (file_exists(pathname))
	{
		fs::path path(pathname);
		strcpy_s(extension, size, path.extension().string().c_str());
	}
}